    ...
    def DownloadStart(self, AllLocalFilesDownload, ALLFIILESIZE):
        ...
        try:
            ...
            for i in AllLocalFilesDownload:
                ...
                self.file_size = client.recv(8)  # 接收文件大小
                self.file_size = struct.unpack("q", self.file_size)[0]
                ...
                with open(i["localpath"], "wb") as f:
                    while True:
                        Rsize = self.file_size - received_size
                        if Rsize == 0:
                            break
                        elif Rsize > flagNum:
                            Rsize = flagNum
        ...
        try:
            ...
            for i in AllLocalFilesUpload:
                if os.path.exists(i["localpath"]):
                    ...
                    self.file_size = os.stat(i["localpath"]).st_size
                    client.send(struct.pack("q", self.file_size))  # 打包发送文件总大小
