...
class myWindow(QFrame):
    ...
    def mouseDoubleClickEvent(self, event):
        # 双击全屏
        if event.button() == Qt.LeftButton and event.pos().x() in self._top_rect[0] and event.pos().y() in self._top_rect[1] and not(self.isMaximized()):
            self.maxWindow()
    ...
    def mouseMoveEvent(self, QMouseEvent):
        ...
        if Qt.LeftButton and self._right_drag:
            ...
            QMouseEvent.accept()
        elif Qt.LeftButton and self._move_drag and QMouseEvent.pos().x() in self._top_rect[0] and QMouseEvent.pos().y() in self._top_rect[1]:
            if not (self.isMaximized()):
                # 标题栏拖放窗口位置
    ...
    def mousePressEvent(self, event):
        ...
        elif (event.button() == Qt.LeftButton) and event.pos().x() in self._top_rect[0] and event.pos().y() in self._top_rect[1]:
            # 鼠标左键点击标题栏区域
            # 当不是最大化时
            if not (self.isMaximized()):
