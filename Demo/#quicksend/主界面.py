...
var_fnums=IntVar()
Scale_foldernums = Scale(setup1,from_=0,to=9,orient=HORIZONTAL,length=200,width=15,resolution=1,tickinterval=3,repeatdelay=100,variable=var_fnums,command=print_foldernums)
...
var_snums=StringVar()
Scale_surfilenums = Scale(setup1,from_=0,to=9,orient=HORIZONTAL,length=200,width=15,resolution=1,tickinterval=3,repeatdelay=100,variable=var_snums,command=print_surfilenums)
...
#运行按钮
Button_run = Button(setup1,text='运行',bg='pink',font=('Arial',12),width=22,height=3,command=run_Helper)   #文本小写！！
Button_run.grid()
...
var_pr=IntVar()
Scale_pcrow = Scale(setup2,from_=1,to=11,orient=HORIZONTAL,length=200,width=15,resolution=1,tickinterval=5,repeatdelay=100,variable=var_pr,command=print_pcrow)
Scale_pcrow.set(4)
Scale_pcrow.grid()
...
var_qr=IntVar()
Scale_qrow = Scale(setup2,from_=1,to=11,orient=HORIZONTAL,length=200,width=15,resolution=1,tickinterval=5,repeatdelay=100,variable=var_qr,command=print_qrow)
Scale_qrow.set(5)
Scale_qrow.grid()
...
var_m0r=IntVar()
Scale_m0row = Scale(setup2,from_=1,to=11,orient=HORIZONTAL,length=200,width=15,resolution=1,tickinterval=5,repeatdelay=100,variable=var_m0r,command=print_m0row)
Scale_m0row.set(3)
Scale_m0row.grid()
Scale_m0row.bind('<<ScaleSelected>>',print_m0row)
Label_m0row= Label(setup2,fg='#00FF00', text='')
Label_m0row.grid()
...
save_Q()
Button_save_Q = Button(setup2,text='保存',font=('Arial',12),width=22,height=3,command=save_Q)   #文本小写！！
Button_save_Q.grid()
...
var_X=IntVar()
Entry_X = Entry(setup3,font=('consolas',14),width=9,textvariable=var_X)
var_X.set('')
...
var_SX=IntVar()
Entry_SX = Entry(setup3,font=('consolas',14),width=9,textvariable=var_SX)
var_SX.set('')
...
save_XY()
Button_save_Q = Button(setup3,text='保存',font=('Arial',12),width=10,height=2,command=save_XY)   #文本小写！！
...
var_rm=IntVar()
Scale_rmax = Scale(setup4,from_=9,to=11,orient=HORIZONTAL,length=200,width=15,resolution=2,tickinterval=2,repeatdelay=100,variable=var_rm,command=print_rmax)
Scale_rmax.set(11)
Scale_rmax.grid()
...
var_f=IntVar()
Scale_f = Scale(setup4,from_=18,to=20,orient=HORIZONTAL,length=200,width=15,resolution=2,tickinterval=2,repeatdelay=100,variable=var_f,command=print_f)
Scale_f.set(20)
Scale_f.grid()
...
save_win()
Button_save_Q = Button(setup4,text='保存',font=('Arial',12),width=10,height=2,command=save_win)
Button_save_Q.grid(column=0, row=5, sticky='S')
