...
window.iconphoto(True, PhotoImage(file=r'.\code\black.png'))  #21059 Administrator
...
def run_Helper():
    ...
    for n in range(1, folders_nums + 1):
        for m in range(0, int(t) + 1):
            ...
            # 第二列nq-3.23
            if n <= (r_max - row_1q+1):
                pg.click(x + c, y + (row_1q-1) * f, duration=0.4)  # nq-data
            else:
                pg.moveTo(s_x, s_y, duration=0.4)
                pg.click(clicks=n -(r_max-row_1q+1), interval=0.4)  # clicks=n-n_max
